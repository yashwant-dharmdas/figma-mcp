// ============================================================
// CommandRegistry — The single source of truth for all Figma MCP commands.
//
// ADDING A NEW COMMAND:
//   1. Define it here in COMMAND_REGISTRY (one entry).
//   2. Implement its handler in packages/plugin/src/handlers/<category>.ts
//   3. Register it in packages/plugin/src/code.ts with dispatcher.register()
//   Everything else (MCP tool, TypeScript types, caching, validation) is automatic.
// ============================================================
...
export const NodeIdSchema = z
  .string()
  .min(1)
  .describe("Figma node ID (e.g. '1:23'). Find it with get_selection or get_node_info.");
...
> {
  /** Unique command name — also becomes the MCP tool name */
  name: string;
  /**
   * Human-readable description shown to the AI model.
   * Be precise: good descriptions directly improve AI tool-calling accuracy.
   * Include: what it does, key params, and when to use vs alternatives.
   */
  description: string;
  category: CommandCategory;
  /** Zod schema for input params. Used for MCP tool registration AND plugin validation. */
  params: TParams;
  /** Zod schema for the expected result shape (used to validate plugin response). */
  result: TResult;
  /**
   * Whether this command requires an active Figma channel.
   * Only false for: join_channel, ping.
   */
  requiresChannel: boolean;
  /**
   * If true, results are cached with TTL to avoid redundant Figma API calls.
   * Only set for read-only commands that don't modify Figma state.
   */
  cacheable?: boolean;
  /** Cache TTL in milliseconds. Only relevant when cacheable=true. Default: 5000ms */
  cacheTtlMs?: number;
  /**
   * Example usages. Shown to the AI to improve tool-calling accuracy.
   * Include at least 1–2 examples for complex commands.
   */
  examples?: Array<{
    description: string;
    // z.input gives the "before default" type so optional fields with
    // .default() don't need to be supplied in example objects.
    input: z.input<TParams>;
  }>;
}

// ── Helper to define a command with full type inference ──────

export function defineCommand<
  TParams extends z.ZodTypeAny,
  TResult extends z.ZodTypeAny,
>(def: CommandDefinition<TParams, TResult>): CommandDefinition<TParams, TResult> {
  return def;
}

// ── COMMAND_REGISTRY ─────────────────────────────────────────

// NOTE: Phase 1 includes foundational + 5 representative commands per category.
// Full 80+ command set is added in Phase 4.

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const COMMAND_REGISTRY: CommandDefinition<any, any>[] = [
  ...
  defineCommand({
    ...
    description:
      "Get metadata about the current Figma document: its name, all pages, and the current page. " +
      "Use this as your first call to orient yourself in the document before making changes. " +
  ...
  defineCommand({
    ...
    description:
      ...
      "Returns node IDs, names, types, and positions. " +
      "Ask the user to select nodes in Figma first if you need to work on specific elements.",
    category: "document",
    params: z.object({}),
    result: z.object({
  ...
  defineCommand({
    ...
    description:
      ...
      "Returns full node properties including fills, strokes, effects, children (for frames/groups), " +
      "typography (for text nodes), and component info. " +
      "Use get_selection first to find node IDs.",
    category: "document",
    params: z.object({
      nodeId: NodeIdSchema,
    }),
  ...
  defineCommand({
    name: "get_nodes_info",
    description:
      "Get detailed information about multiple nodes at once. " +
      "More efficient than calling get_node_info repeatedly. " +
      "Use when you need to inspect several nodes in one step.",
    category: "document",
    params: z.object({
      nodeIds: z.array(NodeIdSchema).min(1).max(50)
        .describe("Array of node IDs to fetch info for (max 50)"),
    }),
  ...
  defineCommand({
    name: "get_styles",
    description:
      "Get all local styles defined in the Figma document: paint styles (colors), " +
      "text styles (typography), effect styles (shadows/blurs), and grid styles. " +
      "Returns style IDs that can be applied to nodes. Cached for 10 seconds.",
    category: "document",
    params: z.object({}),
    result: z.object({
  ...
  defineCommand({
    ...
    description:
      "Scan all text nodes in a frame or the entire page and return their content and IDs. " +
      "Use this to audit text content, find specific strings, or prepare bulk text edits. " +
      "For large frames, results are streamed with progress updates.",
    category: "document",
    params: z.object({
      nodeId: NodeIdSchema.optional().describe("Scope the scan to this node. Omit to scan the whole current page."),
      chunkSize: z.number().int().min(5).max(100).default(20)
        .describe("How many text nodes to process per chunk. Smaller = more frequent progress updates."),
    }),
    result: z.array(z.object({
      id: z.string(),
      name: z.string(),
  ...
  defineCommand({
    ...
    description:
      "Export a node as a PNG, JPG, SVG, or PDF image and return it as a base64-encoded string. " +
      "Use for design previews or asset generation. Large exports may take up to 60 seconds.",
    category: "document",
    params: z.object({
      nodeId: NodeIdSchema,
      ...
      scale: z.number().min(0.5).max(4).default(1)
        .describe("Scale multiplier for raster formats (PNG/JPG). 2 = @2x retina."),
    }),
    result: z.object({
      nodeId: z.string(),
      format: z.string(),
  ...
  defineCommand({
    ...
    description:
      "Get all components defined locally in the current Figma document. " +
      "Returns component keys for use with create_component_instance. " +
  ...
  defineCommand({
    name: "create_frame",
    description:
      "Create a new frame (container) in Figma. Frames are the primary layout containers — " +
      "use them for screens, sections, and component wrappers. " +
      "All coordinates are relative to the parent (or page origin if no parent).",
    category: "creation",
    params: z.object({
    ...
    cacheable: false,
    examples: [
      { description: "Create a 375×812 mobile screen frame", input: { x: 0, y: 0, width: 375, height: 812, name: "Mobile Screen" } },
  ...
  defineCommand({
    ...
    description:
      "Create a rectangle shape. Rectangles are decorative elements — " +
      "use frames instead if you need to contain other elements.",
    category: "creation",
    params: z.object({
      x: z.number(),
      y: z.number(),
  ...
  defineCommand({
    name: "create_text",
    description:
      "Create a text node. Specify content, position, and optional styling. " +
      "For text inside buttons/cards, set parentId to place it inside the frame.",
    category: "creation",
    params: z.object({
      x: z.number(),
      y: z.number(),
      ...
      parentId: NodeIdSchema.optional(),
      fontSize: z.number().min(1).max(1000).optional().default(16).describe("Font size in pixels"),
  ...
  defineCommand({
    ...
    cacheable: false,
    examples: [
      { description: "Create a 48×48 avatar placeholder circle", input: { x: 0, y: 0, width: 48, height: 48, fillColor: { r: 0.9, g: 0.9, b: 0.9, a: 1 } } },
    ],
  }),
  ...
  defineCommand({
    name: "clone_node",
    description:
      "Duplicate an existing node. The clone appears next to the original by default, " +
      "or at the specified x/y position.",
    category: "creation",
    params: z.object({
      nodeId: NodeIdSchema,
  ...
  defineCommand({
    ...
    description:
      "Combine multiple vector shapes using a Boolean operation. " +
      "Union merges shapes, Subtract removes overlap, Intersect keeps only the overlap, Exclude inverts.",
    category: "creation",
    params: z.object({
  ...
  defineCommand({
    ...
    description:
      ...
      "Use r/g/b/a values in 0–1 range (not 0–255). " +
      "To convert hex: #FF0000 → r:1, g:0, b:0. " +
      "Set a:0 for fully transparent, a:1 for fully opaque.",
  ...
  defineCommand({
    ...
    description:
      "Set the stroke (border) color and weight of a node. " +
      "Use r/g/b/a values in 0–1 range.",
    category: "modification",
    params: z.object({
      nodeId: NodeIdSchema,
  ...
  defineCommand({
    ...
    description:
      "Recursively update all fill colors that match a source color inside a node tree. " +
      "Useful for rebranding — replace all instances of one color with another. " +
      "Processes nodes in chunks with progress updates for large selections.",
    category: "modification",
    params: z.object({
      ...
      tolerance: z.number().min(0).max(0.1).optional().default(0.01)
        .describe("Color matching tolerance (0–0.1). Higher = more fuzzy matching."),
    }),
  ...
  defineCommand({
    ...
    description:
      "Configure auto-layout on a frame. Auto-layout arranges children automatically like CSS flexbox. " +
  ...
  defineCommand({
    name: "set_effects",
    description:
      "Set visual effects on a node: drop shadows, inner shadows, layer blurs, or background blurs. " +
      "Replaces all existing effects.",
    category: "modification",
    params: z.object({
      nodeId: NodeIdSchema,
  ...
  defineCommand({
    ...
    params: z.object({
      ...
      relative: z.boolean().optional().default(false)
        .describe("If true, adds the angle to the current rotation. If false, sets an absolute rotation."),
    }),
  ...
  defineCommand({
    name: "set_image",
    description:
      "Fill a node with an image from a URL or base64 data. " +
    ...
    params: z.object({
      ...
      scaleMode: z.enum(["FILL", "FIT", "CROP", "TILE"]).optional().default("FILL")
        .describe("How the image fills the node"),
    }),
    result: z.object({ id: z.string() }),
    requiresChannel: true,
    cacheable: false,
  ...
  defineCommand({
    ...
    params: z.object({
      ...
      position: z.enum(["FRONT", "BACK", "FORWARD", "BACKWARD", "INDEX"])
        .describe("FRONT/BACK = move to top/bottom. FORWARD/BACKWARD = move one step. INDEX = move to specific index."),
  ...
  defineCommand({
    ...
    description:
      "Update text content for multiple text nodes at once. " +
      "More efficient than calling set_text_content in a loop. " +
      "Maps node IDs to their new text values.",
    category: "text",
    params: z.object({
  ...
  defineCommand({
    ...
    params: z.object({
      nodeId: NodeIdSchema,
      fontSize: z.number().min(1).max(1000).describe("Font size in pixels"),
    }),
    result: z.object({ id: z.string() }),
    requiresChannel: true,
    cacheable: false,
  ...
  defineCommand({
    ...
    params: z.object({
      nodeId: NodeIdSchema,
      weight: z.enum(["Thin", "ExtraLight", "Light", "Regular", "Medium", "SemiBold", "Bold", "ExtraBold", "Black"])
        .describe("Font weight name"),
    }),
    result: z.object({ id: z.string() }),
    requiresChannel: true,
    cacheable: false,
  ...
  defineCommand({
    ...
    params: z.object({
      ...
      textCase: z.enum(["ORIGINAL", "UPPER", "LOWER", "TITLE"])
        .describe("ORIGINAL = no transformation. UPPER/LOWER/TITLE = force case."),
    }),
    result: z.object({ id: z.string() }),
    requiresChannel: true,
    cacheable: false,
  ...
  defineCommand({
    ...
    description:
      "Get the styled segments of a mixed-format text node (e.g. bold + regular in same node). " +
      "Returns per-segment style properties.",
    category: "text",
    params: z.object({
      nodeId: NodeIdSchema,
      property: z.enum(["fontSize", "fontName", "fills", "letterSpacing", "lineHeight", "paragraphSpacing", "textCase", "textDecoration", "textStyleId", "listOptions"])
        .describe("Which style property to inspect across segments"),
    }),
    result: z.array(z.object({
      characters: z.string(),
      start: z.number(),
  ...
  defineCommand({
    ...
    description:
      "Pre-load a font into the Figma document before using it. " +
      "Required before setting fonts that are not already in the document.",
    category: "text",
    params: z.object({
  ...
  defineCommand({
    ...
    description:
      "Create an instance of a component (local or from a team library). " +
      "Use get_local_components to find component keys.",
    category: "component",
    params: z.object({
  ...
  defineCommand({
    ...
    params: z.object({
      ...
      properties: z.record(z.string(), z.string())
        .describe("Variant property key-value pairs (e.g. { State: 'Hover', Size: 'Large' })"),
    }),
    result: z.object({ id: z.string() }),
    requiresChannel: true,
    cacheable: false,
  ...
  defineCommand({
    name: "set_svg",
    description:
      "Insert an SVG as a vector node on the canvas. " +
      "The SVG string must be valid XML. Maximum size: 500KB.",
    category: "svg",
    params: z.object({
  ...
  defineCommand({
    name: "get_variables",
    description:
      "Get all Figma Variables (design tokens) defined in the document: colors, numbers, strings, booleans. " +
      "Returns variable IDs needed for apply_variable_to_node.",
    category: "variable",
    params: z.object({}),
    result: z.object({
  ...
  defineCommand({
    ...
    description:
      ...
      "This links the node's property to the design token so it updates when the variable changes.",
    category: "variable",
    params: z.object({
      nodeId: NodeIdSchema,
  ...
  defineCommand({
    name: "batch_execute",
    description:
      "Execute multiple Figma commands in a single round-trip — the most efficient way to make bulk changes. " +
      "ALWAYS prefer this over calling individual tools in a loop. " +
      "Example use cases: change 20 colors, create a full UI layout, update multiple text nodes. " +
      "Set stopOnError=false to collect all results even when some operations fail (non-atomic). " +
      "Set stopOnError=true (default) to stop on first failure.",
    category: "batch",
    params: z.object({
      ...
      stopOnError: z.boolean().optional().default(true)
        .describe("Stop on first error (true) or collect all results (false)"),
    }),
    result: z.object({
...
/** O(1) lookup map: command name → definition */
export const COMMAND_MAP = new Map(
  COMMAND_REGISTRY.map((def) => [def.name, def])
);

/** Union type of all valid command names */
export type FigmaCommandName = (typeof COMMAND_REGISTRY)[number]["name"];

/** All commands in a given category */
export function getCommandsByCategory(category: CommandCategory) {
  return COMMAND_REGISTRY.filter((def) => def.category === category);
}

/** Get a command definition by name, throws if not found */
export function getCommand(name: string): CommandDefinition {
  const def = COMMAND_MAP.get(name);
  if (!def) throw new Error(`Unknown command: "${name}". Valid commands: ${[...COMMAND_MAP.keys()].join(", ")}`);
  return def;
}
